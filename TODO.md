...
- 留存模型：B 模式
  ...
  - 热窗口证据：截图/音频（默认 365 天，可配置，可清理；超过阈值仅提醒不自动清理）
...
- [ ] M1-5 轻量 Benchmark（随时跑/回归；先做本地版）
  - 背景：压实/清洗/检索是“基础能力”，规则迭代很容易出现回退；没有自动化评估就无法知道效果是否真的变好。
  - 约束：必须轻量、离线、可随时跑；默认不上传任何原文/截图（仅输出统计结果）。
  - 验收：
    - 一条命令产出一份报告（markdown/JSON 均可），包含：chunk 清洗前后长度/去重/噪声占比等核心指标 + 检索 TopK 的稳定性/可读性指标。
    - 支持两种数据源：
      - `synthetic`：仓库内置小型合成噪声样本（可用于 CI/回归）
      - `local`：读取本机 SQLite（只做统计与抽样，不做外发），可快速验证“我的真实数据”是否改善
    - 跑完可对比上一次结果（同机同库），能一眼看出是否回退。
