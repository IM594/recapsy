...
- [ ] M1-4 OCR/压实质量提升（让 chunks 可用）
  ...
  - 子任务（建议优先）：
    - [ ] 提供“重建 chunks”维护命令（仅重建派生数据）
      - 背景：压实算法迭代后，历史 chunks 仍然是旧噪声；只优化新数据不够用。
      - 验收：可一键删除并重建 chunks/FTS（frames 保留）；重建后搜索质量明显改善。
    - [ ] 压实清洗参数化（可调强度）
...
- [ ] 采集质量提升（后续：前台窗口截图 / text-first）
  ...
  - 方向：优先截取前台窗口区域（CGWindowListCreateImage）；再考虑 AX 可见文本（更像 Littlebird）。
