...
const captureBundlePath = path.join(contentsPath, 'Frameworks', 'RecapsyCapture.app');

describe('packaged Electron application layout', () => {
  it('produces a real Recapsy application with the frozen outer bundle id', () => {
    expect(existsSync(packagedAppPath)).toBe(true);
    expect(readBundleIdentifier(packagedAppPath)).toBe('one.recapsy.desktop');
    expect(() =>
  ...
  it('embeds valid independently signed capture code in the nested-code directory', async () => {
    ...
    const capturePath = path.join(captureBundlePath, 'Contents', 'MacOS', 'Recapsy');

    expect(readBundleIdentifier(captureBundlePath)).toBe('one.recapsy.desktop.capture');
    expect(existsSync(path.join(resourcesPath, 'Recapsy.app'))).toBe(false);
  ...
  it('enforces the configured signing-channel policy and matches the host architecture', () => {
    const outerExecutablePath = path.join(contentsPath, 'MacOS', 'Recapsy');
    const launcherPath = path.join(captureBundlePath, 'Contents', 'MacOS', 'CaptureLauncher');
    const capturePath = path.join(captureBundlePath, 'Contents', 'MacOS', 'Recapsy');
...
function readBundleIdentifier(bundlePath: string): string {
