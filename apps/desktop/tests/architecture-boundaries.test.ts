...
describe('desktop architecture boundaries', () => {
  it('keeps TypeScript tests inside owned tests directories', async () => {
    ...
    for await (const relativePath of glob.scan({ cwd: DESKTOP_PACKAGE_ROOT })) {
      if (relativePath.includes('/__tests__/')) {
        violations.push(`${relativePath} uses the retired __tests__ convention`);
        continue;
      }
      if (!/[.](?:test|spec)[.]ts$/.test(relativePath)) continue;
      if (
        relativePath.startsWith('src/') &&
        !/^src\/(?:[^/]+\/)+tests\/.+\.test\.ts$/.test(relativePath)
      ) {
        violations.push(`${relativePath} is outside its owning module tests directory`);
        continue;
      }
      if (relativePath.endsWith('.spec.ts')) {
        violations.push(`${relativePath} uses the retired spec filename convention`);
        continue;
      }
      if (relativePath.startsWith('integration/')) {
        violations.push(`${relativePath} must move under tests/integration`);
      }
    }

    const sourceGlob = new Bun.Glob('**/*.ts');
    for await (const relativePath of sourceGlob.scan({ cwd: DESKTOP_SOURCE_ROOT })) {
      if (relativePath.includes('/tests/') || /[.](?:test|spec)[.]ts$/.test(relativePath)) {
        continue;
      }
      ...
      for (const match of source.matchAll(TEST_IMPORT_PATTERN)) {
        ...
        if (targetPath.split('/').includes('tests') || /[.](?:test|spec)$/.test(targetPath)) {
          violations.push(`${relativePath} imports test code through ${specifier}`);
        }
      }
    }
  ...
  it('routes cross-module test imports through capability entrypoints', async () => {
    const violations: string[] = [];
    const moduleTests = new Bun.Glob('**/tests/**/*.test.ts');

    for await (const relativePath of moduleTests.scan({ cwd: DESKTOP_SOURCE_ROOT })) {
      ...
      for (const match of source.matchAll(IMPORT_PATTERN)) {
        ...
        if (!targetCapability || targetCapability === sourceCapability) continue;
        if (targetPath !== targetCapability && targetPath !== `${targetCapability}/index`) {
          violations.push(`${relativePath} -> ${specifier}`);
        }
      }
    }

    const integrationTests = new Bun.Glob('**/*.test.ts');
    for await (const relativePath of integrationTests.scan({ cwd: DESKTOP_INTEGRATION_ROOT })) {
      const source = await readFile(path.resolve(DESKTOP_INTEGRATION_ROOT, relativePath), 'utf8');
      for (const match of source.matchAll(TEST_IMPORT_PATTERN)) {
        const specifier = match[1];
        if (specifier?.startsWith('../../src/')) {
          const targetPath = resolveLocalImport(`tests/integration/${relativePath}`, specifier);
          const sourcePath = targetPath.replace(/^src\//, '');
          const targetCapability = capabilityForPath(sourcePath);
          if (!targetCapability) continue;
          const isTestInfrastructureAdapter = sourcePath === 'storage/sqlite/bun';
          if (sourcePath !== `${targetCapability}/index` && !isTestInfrastructureAdapter) {
            violations.push(`tests/integration/${relativePath} -> ${specifier}`);
          }
          continue;
        }
        if (
  ...
  it('gives the single-job sync state machine its own executor', async () => {
    ...
    const violations: string[] = [];
    for (const requiredPath of ['sync/job.ts', 'sync/tests/job.test.ts']) {
      if (!sourceFiles.includes(requiredPath)) {
        violations.push(`${requiredPath} is missing`);
      }
    }
  ...
  it('gives retry policy its own sync module and test ownership', async () => {
    ...
    if (!sourceFiles.includes('sync/tests/retry.test.ts')) {
      violations.push('sync/tests/retry.test.ts is missing');
    }
    if (sourceFiles.includes('sync/tests/retry-backoff.test.ts')) {
      violations.push('sync/tests/retry-backoff.test.ts is still present');
    }
  ...
  it('owns sync error classification and safe presentation in one module', async () => {
    ...
    const violations: string[] = [];
    for (const requiredPath of ['sync/errors.ts', 'sync/tests/errors.test.ts']) {
      if (!sourceFiles.includes(requiredPath)) {
        violations.push(`${requiredPath} is missing`);
      }
    }
  ...
  it('owns sync queue summary projection in a narrow read module', async () => {
    ...
    const violations: string[] = [];
    for (const requiredPath of ['sync/summary.ts', 'sync/tests/summary.test.ts']) {
      if (!sourceFiles.includes(requiredPath)) {
        violations.push(`${requiredPath} is missing`);
      }
    }
  ...
  it('separates server-capture reconciliation from storage asset reconciliation', async () => {
    ...
    for (const requiredPath of [
      ...
      'sync/tests/reconciliation.test.ts',
      'storage/reconciliation.ts',
      'storage/tests/reconciliation.test.ts',
    ]) {
      if (!sourceFiles.includes(requiredPath)) {
        violations.push(`${requiredPath} is missing`);
      }
  ...
  it('splits helper protocol responsibilities and binds process directions explicitly', async () => {
    ...
    const requiredPaths = [
      ...
      'helper/protocol/tests/validation.test.ts',
    ...
    for (const relativePath of sourceFiles) {
      if (relativePath.startsWith('helper/')) {
        continue;
      }
      const source = await readSource(relativePath);
    ...
    for await (const relativePath of glob.scan({ cwd: DESKTOP_INTEGRATION_ROOT })) {
      const source = await readFile(path.resolve(DESKTOP_INTEGRATION_ROOT, relativePath), 'utf8');
      if (/from\s+['"][^'"]*helper\/protocol(?:\/|['"])/.test(source)) {
