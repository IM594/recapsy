...
public enum CaptureAsset {
    ...
    public static let screenshotFileName = "screenshot.jpg"

    public static let screenshotMimeType = "image/jpeg"
    ...
    /// must be a stable `<captureId>/screenshot.jpg`.
    public static func screenshotRelativeKey(captureId: String) -> String {
        return "\(captureId)/\(screenshotFileName)"
    }

    /// Absolute on-disk destination for a capture's JPEG, derived from the
