 ...
 * process runs on Node (not Bun), so `bun-sqlite-driver.ts` (which imports
 ...
 * mirrors `bun-sqlite-driver.ts` structurally.
 *
 ...
 * compatible with `bun-sqlite-driver.ts`). SQLite itself has no boolean
