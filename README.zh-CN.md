...
Recapsy 记录你的 Mac 屏幕，识别上面的文字，全部留在你的电脑里。你已经在用的 agent，Claude Code、Cursor、Codex 或任何会说 MCP 的东西，问 Recapsy 你刚才在做什么，拿到的回答附带截图、原文和时间。

- **桌面端永远免费、永远开源。** 一个字节都不出电脑。
- **云端档订阅制，用来养这个项目。** 同步、托管模型、备份、手机上问，这些收费。
- **免费过的东西永远免费。**

## 为什么

开会前十分钟，你让 agent 问一句“我这周做了什么”，拿到带细节的发言稿：哪些文件、哪几个对话、哪个下午。没有它，你要在 git、Slack 和 Notion 里翻一个小时。年终总结同理，更日常的一幕是 agent 开工前先问你在这个项目干到哪了。

## 它不会骗你

录屏工具会漏采、会压缩、会悄悄删。Recapsy 承诺四条底线，它们一条都不敢。
...
每帧都跑的事全在你的 Mac 上跑，零成本。总结每天跑一次，用你自己配的模型，任何 OpenAI 兼容地址或本机 Ollama 都行，不配也不影响其他功能。随时想问的事交给你自己的 agent 走 MCP，所以没有聊天界面。识字用 PP-OCR 本机运行，调度目标是活动监视器能耗一直显示“低”。只支持 macOS。
