...
- **Vitest for tests.** Tests run on the same Node major as the Electron main process, so module semantics match. Electron ships its own ABI, so loading native modules is verified inside Electron on macOS, not by these tests. Vitest also covers the renderer later.
